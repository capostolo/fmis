...
<div class='form-group col-3' > 
          ...
          <select class='form-control selectpicker' name='fertiliser_id' id='fertiliser_id' data-live-search='true' data-style='' data-style-base='form-control' data-virtual-scroll='200' required>
            ...
            <option value='<?= $r->id ?>' <?= set_select('fertiliser_id', $r->id, $r->id == $row->fertiliser_id) ?>> <?= $r->fertiliser_description ?> </option>
            <?php } ?>
          </select>
        </div> 
<div class='form-group col-3' > 
          ...
          <input type='text' class='form-control ' name='quantity' id='quantity' value='<?= set_value('quantity', $row->quantity) ?>' required />
        </div> 
<div class='form-group col-3' > 
      ...
      <table class="table table-striped dstable text-custom-anthrax">
        ...
        <tbody>
         <?php foreach($crops As $f){?>
           <tr>
            <td>
              <input type='checkbox' class="selectMe" name='fi_selected[]' value="<?= $f->parcel_id ?>" <?= ($f->fertilisation_parcel_id) ? 'checked' : '' ?>/>
            </td>
