...
<?php
  $attributes = array('class' => 'form', 'id' => 'update-item');
	echo form_open(site_url('fmis/spray-bulk'), $attributes);
	?>
	<div class='row'>
    ...
    <div class='form-group col-4' > 
      ...
      <input type='text' class='form-control ' name='days_before_harvest' id='days_before_harvest' value='<?= set_value('days_before_harvest', $row->days_before_harvest) ?>' required <?= $disabled ?> />
    </div> 
    ...
    <div class='form-group col-3' > 
      ...
      <input type='text' class='form-control ' name='dose' id='dose' value='<?= set_value('dose', $row->dose) ?>' required <?= $disabled ?> />
    </div> 
    <div class='form-group col-3' > 
    ...
    <div class='form-group col-6'> 
      ...
      <textarea class='form-control' name='target' id='target' required <?= $disabled ?> > <?= set_value('target', $row->target) ?></textarea>
    </div> 
    <div class='form-group col-6'> 
      <label class='control-label' for='conditions'><?= lang('Fmis.conditions') ?></label>
      <textarea class='form-control' name='conditions' id='conditions' required <?= $disabled ?> > <?= set_value('conditions', $row->conditions) ?></textarea>
    </div> 
      ...
      <table class="table table-striped dstable text-custom-anthrax">
        ...
        <tbody>
         <?php foreach($crops As $f){?>
           <tr>
            <td>
              <input type='checkbox' class="selectMe" name='fi_selected[]' value="<?= $f->parcel_id ?>" <?= ($f->spray_parcel_id) ? 'checked' : '' ?> <?= $disabled ?>/>
            </td>
